...
The firmware itself: an alternate firmware for the Nava, a replica of the Roland
TR-909. It enhances the original with improved pattern chaining, button logic,
metronome and external instrument (MIDI) functionality.

This file is the design record - the reasoning that is not visible in the sources.
For what each file contains and when to open it, see `CLAUDE.md` beside this one.
